...
function PredictionView({predict}){
  const listItems = predict.map((item, i) =>
    <Text key={i} style={styles.predict}>{item.className} : {(item.probability*100).toFixed(3)}%</Text>
  );
  return (
    <View>
      {listItems}
...
export default function ImageClassificationScreen({ navigation }) {
  ...
  async function doClassification() {
    ...
    if (model) {
      ...
      const predict = await model.classify(imageTensor);
